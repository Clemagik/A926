...
static unsigned long ingenic_pll_recalc_rate(struct clk_hw *hw, unsigned long parent_rate)
{
	...
	regmap_read(iclk->pm_regmap, hwdesc->regoff, &pll);

	if (hwdesc->on_bit != -1 && !(pll & BIT(hwdesc->on_bit)))
		return 0;

	od = (pll >> hwdesc->od_sft) & GENMASK(hwdesc->od_width - 1, 0);
	n = (pll >> hwdesc->n_sft) & GENMASK(hwdesc->n_width - 1, 0);
	m = (pll >> hwdesc->m_sft) & GENMASK(hwdesc->m_width - 1, 0);
