						  ...
						  unsigned long parent_rate)
{
	...
	if (divider->flags & CLK_DIVIDER_QUADRUPLE) {
		ret = cgu_div->div_ops->recalc_rate(&cgu_div->div.hw, parent_rate / 4);
	} else {
		ret = cgu_div->div_ops->recalc_rate(&cgu_div->div.hw, parent_rate);
	}
					...
					unsigned long *prate)
{
	...
	maxdiv = _get_maxdiv(divider->table, divider->width, divider->flags);

	if (rate <= (*prate / (maxdiv * 4)))
		bestrate = *prate / (maxdiv * 4);
	else
		bestrate = cgu_div->div_ops->round_rate(&cgu_div->div.hw, rate, prate);
	return bestrate;
}
...
static int clk_cgu_divider_set_rate(struct clk_hw *hw, unsigned long rate, unsigned long parent_rate)
{
	...
	if (divider->flags & CLK_DIVIDER_QUADRUPLE) {
		ret = cgu_div->div_ops->set_rate(&cgu_div->div.hw, rate * 4, parent_rate);
	} else {
