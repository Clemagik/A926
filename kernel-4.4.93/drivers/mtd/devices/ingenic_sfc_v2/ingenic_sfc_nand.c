...
static int32_t ingenic_sfcnand_read_oob(struct mtd_info *mtd, loff_t from, struct mtd_oob_ops *ops)
{
	...
	if(ops->datbuf) {
		/* create DMA Descriptors */
		ret = create_sfc_desc(flash, (unsigned char *)ops->datbuf, ops->len);
		if(ret < 0){
			dev_err(flash->dev, "%s create descriptors error. -%d\n", __func__, ret);
			goto read_oob_exit;
		}
		...
		ret = ingenic_sfc_nand_read(flash, addr / mtd->writesize, addr % mtd->writesize, ops->datbuf, ops->len);
		if(ret < 0) {
	...
	if(ops->oobbuf) {
		...
		ret = ingenic_sfc_nand_read(flash, addr / mtd->writesize, mtd->writesize + ops->ooboffs, ops->oobbuf, ops->ooblen);
		if(ret < 0)
...
static void create_cdt_table(struct sfc_flash *flash, uint32_t flag)
{
	...
	if(flag & UPDATE_CDT){
		...
		if (!(flag & DEFAULT_CDT)) {
			...
			write_cdt(flash->sfc, sfc_cdt, NAND_RESET, NAND_ECC_STATUS_READ);
		}
	}
	//dump_cdt(flash->sfc);
}
...
static int ingenic_sfc_resume(struct platform_device *pdev)
{
	...
	create_cdt_table(flash, DEFAULT_CDT | UPDATE_CDT);
