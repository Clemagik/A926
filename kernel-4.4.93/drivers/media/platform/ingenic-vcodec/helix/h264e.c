...
//	printk("s->emc_bs_va = 0x%08x\n", s->emc_bs_va);
	...
	printk("s->bs_head_en = %d\n", s->bs_head_en);
	printk("s->bs_head_va = 0x%08x\n", (unsigned int)s->bs_head_va);
	printk("s->bs_head_len = 0x%d\n", (unsigned int)s->bs_head_len);
	printk("s->bs_rbsp_en = %d\n", s->bs_rbsp_en);
...
static int h264e_fill_slice_info(struct h264e_ctx *ctx, unsigned int sh_size_in_bits)
{
	...
	/* 1. rc output [11] */
	{
		...
		s->frame_type 			= ctx->sh.i_type == SLICE_TYPE_I ? 0:1;
		s->first_mby 			= 0;
		s->last_mby 			= s->mb_height - 1;
		s->frame_width 			= ctx->p.width;
		s->frame_height 		= ctx->p.height;
		s->qp				= ctx->sh.i_type == SLICE_TYPE_I ? ctx->p.i_qp : ctx->p.p_qp;
		s->base_qp			= s->qp;
...
int h264e_alloc_workbuf(struct h264e_ctx *ctx)
{
	...
	ctx->desc = dma_alloc_noncoherent(NULL, ctx->vdma_chain_len, &ctx->desc_pa, GFP_KERNEL);
	if(!ctx->desc) {
...
err_fb:
	...
	dma_free_noncoherent(NULL, ctx->vdma_chain_len, ctx->desc, ctx->desc_pa);
err_desc:
	kfree(ctx->bs_header);
	ctx->bs_header = NULL;
