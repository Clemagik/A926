...
static int devmem_alloc(devmem_ctx_t *ctx, devmem_info_t *di)
{
	...
	if(di->coherent) {
		di->kva = (unsigned long)dma_alloc_coherent(NULL, di->length, &dma_addr, GFP_KERNEL);
	} else {
		di->kva = (unsigned long)dma_alloc_noncoherent(NULL, di->length, &dma_addr, GFP_KERNEL);
	}
...
{
	if(di->coherent) {
		dma_free_coherent(NULL, di->length, (void *)di->kva, (dma_addr_t)di->paddr);
	} else {
		dma_free_noncoherent(NULL, di->length, (void *)di->kva, (dma_addr_t)di->paddr);
	}
}
...
static void devmem_sync(devmem_ctx_t *ctx, devmem_info_t *di)
{
	...
	dma_cache_sync(NULL, (void *)di->kva, di->length, di->sync_dir);
...
AVBuffer *av_buffer_alloc(unsigned int size)
{
...
void av_buffer_sync(AVBuffer *buf, int dir)
{
	buf->di.sync_dir = dir;

	devmem_sync(NULL, &buf->di);
}


void av_buffer_free(AVBuffer *buf)
{
	if(!buf)
		return;
