...
static void release_picture(H264Context *h, H264Picture *pic)
{
	...
	av_buffer_free(pic->frm_info_ctrl);
	av_buffer_free(pic->frm_info_mv);
	if(h->format == VPU_FORMAT_NV12 || h->format == VPU_FORMAT_NV21) {
		av_buffer_free(pic->dec_result_y);
		av_buffer_free(pic->dec_result_uv);
	}
}
...
static int alloc_picture(H264Context *h, H264Picture *pic)
{
    ...
    pic->frm_info_ctrl = av_buffer_alloc(h->width * h->height * 3 / 2);
    if(!pic->frm_info_ctrl){
	av_log(NULL, AV_LOG_ERROR, "Failed to alloc frm_info_ctrl!\n");
	ret = AVERROR(ENOMEM);
	goto err_frm_info_ctrl;
    }
    pic->frm_info_mv = av_buffer_alloc(h->width * h->height * 3 / 2);
    if(!pic->frm_info_mv) {
    ...
    if(h->format == VPU_FORMAT_NV12 || h->format == VPU_FORMAT_NV21) {
	pic->dec_result_y = av_buffer_alloc(h->width * h->height);
	if(!pic->dec_result_y) {
		av_log(NULL, AV_LOG_ERROR, "aFailed to alloc dec_result_y!\n");
		ret = AVERROR(ENOMEM);
		goto err_dec_result_y;
	}
	pic->dec_result_uv = av_buffer_alloc(h->width * h->height / 2);
...
#define ROUND_UP(x, align) (((int) (x) + (align - 1)) & ~(align - 1))
    ...
    av_buffer_sync(h->dma_desc, DMA_TO_DEVICE);
    //av_buffer_sync(h->pkt.rbsp_buf, DMA_TO_DEVICE);
