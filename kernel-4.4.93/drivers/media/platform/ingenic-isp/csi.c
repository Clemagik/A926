...
static void csi_dump_regs(struct csi_device *csi)
{
	dev_info(csi->dev, "VERSION		:0x%08x\n",	csi_core_readl(csi, VERSION));
	dev_info(csi->dev, "N_LANES		:0x%08x\n",	csi_core_readl(csi, N_LANES));
	dev_info(csi->dev, "PHY_SHUTDOWNZ	:0x%08x\n",	csi_core_readl(csi, PHY_SHUTDOWNZ));
	dev_info(csi->dev, "DPHY_RSTZ		:0x%08x\n",	csi_core_readl(csi, DPHY_RSTZ));
	dev_info(csi->dev, "CSI2_RESETN		:0x%08x\n",	csi_core_readl(csi, CSI2_RESETN));
	dev_info(csi->dev, "PHY_STATE		:0x%08x\n",	csi_core_readl(csi, PHY_STATE));
	dev_info(csi->dev, "DATA_IDS_1		:0x%08x\n",	csi_core_readl(csi, DATA_IDS_1));
	dev_info(csi->dev, "DATA_IDS_2		:0x%08x\n",	csi_core_readl(csi, DATA_IDS_2));
	dev_info(csi->dev, "ERR1		:0x%08x\n",	csi_core_readl(csi, ERR1));
	dev_info(csi->dev, "ERR2		:0x%08x\n",	csi_core_readl(csi, ERR2));
	dev_info(csi->dev, "MASK1		:0x%08x\n",	csi_core_readl(csi, MASK1));
	dev_info(csi->dev, "MASK2		:0x%08x\n",	csi_core_readl(csi, MASK2));
	dev_info(csi->dev, "PHY_TST_CTRL0       :0x%08x\n",	csi_core_readl(csi, PHY_TST_CTRL0));
	dev_info(csi->dev, "PHY_TST_CTRL1       :0x%08x\n",	csi_core_readl(csi, PHY_TST_CTRL1));
	dev_info(csi->dev, "VC0_FRAME_NUM	:0x%08x\n",	csi_core_readl(csi, VC0_FRAME_NUM));
	dev_info(csi->dev, "VC1_FRAME_NUM	:0x%08x\n",	csi_core_readl(csi, VC1_FRAME_NUM));
	dev_info(csi->dev, "VC2_FRAME_NUM	:0x%08x\n",	csi_core_readl(csi, VC2_FRAME_NUM));
	dev_info(csi->dev, "VC3_FRAME_NUM	:0x%08x\n",	csi_core_readl(csi, VC3_FRAME_NUM));
	dev_info(csi->dev, "VC0_LINE_NUM	:0x%08x\n",	csi_core_readl(csi, VC0_LINE_NUM));
	dev_info(csi->dev, "VC1_LINE_NUM	:0x%08x\n",	csi_core_readl(csi, VC1_LINE_NUM));
	dev_info(csi->dev, "VC2_LINE_NUM	:0x%08x\n",	csi_core_readl(csi, VC2_LINE_NUM));
	dev_info(csi->dev, "VC3_LINE_NUM	:0x%08x\n",	csi_core_readl(csi, VC3_LINE_NUM));

	dev_info(csi->dev, "CSI_PHY_1C2C_MODE	:0x%08x\n", 	csi_phy_readl(csi, CSI_PHY_1C2C_MODE));
...
static inline csi_core_write_io(unsigned int iobase, unsigned int address, unsigned int data, unsigned char shift, unsigned char width)
{
        unsigned int mask = (1 << width) - 1;
...
static int csi_subdev_s_stream(struct v4l2_subdev *sd, int enable)
{
	...
	if(enable) {
		csi_phy_writel(csi, CSI_PHY_1C2C_MODE, 1);	/*FIX to 2c mode.*/
		csi_core_writel(csi, N_LANES, 1);		/*FIX to 2 lane.*/
...
static int __maybe_unused ingenic_csi_runtime_suspend(struct device *dev)
{
        return 0;
}

static int __maybe_unused ingenic_csi_runtime_resume(struct device *dev)
{
        return 0;
}

static const struct dev_pm_ops ingenic_csi_pm_ops = {
        SET_SYSTEM_SLEEP_PM_OPS(pm_runtime_force_suspend,
                                pm_runtime_force_resume)
        SET_RUNTIME_PM_OPS(ingenic_csi_runtime_suspend, ingenic_csi_runtime_resume, NULL)
};
