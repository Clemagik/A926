		...
		struct ov772x_priv *priv)
{
	...
	int interfac_vol = 0, ret = 0;

	ret = of_property_read_u32(np, "ingenic,cim-interface-vol", &interfac_vol);
	if(!ret) {
		if(interfac_vol == CIM_INTERFACE_VOL_1V8)
		{
			printk("1.8v\n");
			*(volatile unsigned int *) 0xb00000E0 |= (1 << 30);
		}
		if(interfac_vol == CIM_INTERFACE_VOL_3V3)
		{
			printk("3.3v\n");
			*(volatile unsigned int *) 0xb00000E0 &= ~(1 << 30);
		}
	} else {
		dev_info(&client->dev, "cim interface is the default voltage\n");
	}
