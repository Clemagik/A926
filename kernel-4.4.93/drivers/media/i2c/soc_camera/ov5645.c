...
static const struct regval_list ov5645_vga_regs[] = {
	...
	{0x3008, 0x42}, // lens off, bpc on, wpc on
	{0x3008, 0x11}, // lens off, bpc on, wpc on
	{0x3103, 0x03}, //
	{0x3503, 0x07}, // D[9:8] output
	{0x3002, 0x1c}, // D[7:0] output
	{0x3006, 0xc3}, // Vsync, Href, PCLK, Frex, Strobe, SDA, GPIO1, GPIO0 output
	{0x300e, 0x45}, // two data lane
//	{0x300e, 0x25}, // one data lane

	{0x3017, 0x40}, // gain ceiling = 15.5x
	{0x3018, 0x00}, // gain ceiling
	{0x302e, 0x0b}, // band detection manual mode
	{0x3037, 0x13}, // strobe frex mode
	{0x4837, 0x11},
	{0x3108, 0x01},
	{0x3611, 0x06},
	{0x3612, 0xab},
...
#if 1
	/* : ns*/
	{0x4818, 0x00}, /*MIN HS ZERO H*/
	{0x4819, 0x96}, /*MIN HS ZERO L*/
	{0x481a, 0x00}, /*MIN MIPIHS TRAIL H*/
	{0x481b, 0x3c}, /*MIN MIPIHS TRAIL L*/
	{0x481c, 0x01}, /*MIN MIPI CLK ZERO H*/
	{0x481d, 0x86}, /*MIN MIPI CLK ZERO L*/
	{0x481e, 0x00}, /*MIN MIPI CLK PREPARE H*/
	{0x481f, 0x3C}, /*MIN MIPI CLK PREPARE L*/
	{0x4820, 0x00}, /*MIN CLK POST H*/
	{0x4821, 0x56}, /*MIN CLK POST L*/
	{0x4822, 0x00}, /*MIN CLK TRAIL H*/
	{0x4823, 0x3C}, /*MIN CLK TRAIL L*/
	{0x4824, 0x00}, /*MIN LPX PCLK H*/
	{0x4825, 0x00}, /*MIN LPX PCLK L*/
	{0x4826, 0x00}, /*MIN HS PREPARE H*/
	{0x4827, 0x32}, /*MIN HS PREPARE L*/
//	{0x4828, 0x00}, /*MIN HS EXIT H*/
//	{0x4829, 0x64}, /*MIN HS EXIT L*/

	/* : ui*/
	{0x482a, 0x05}, /*MIN HS ZERO/UI*/
	{0x482b, 0x04}, /*MIN HS TRAIL/UI*/
	{0x482c, 0x00}, /*MIN CLK ZERO/UI*/
	{0x482d, 0x00}, /*MIN CLK PREPARE/UI*/
	{0x482e, 0x34}, /*MIN CLK POST/UI*/
	{0x482f, 0x00}, /*MIN CLK TRAIL/UI*/
	{0x4830, 0x00}, /*MIN LPX PCLK/UI*/
	{0x4831, 0x04}, /*MIN HS PREPARE/UI*/
	{0x4832, 0x00}, /*MIN HS EXIT/UI*/

//	{0x4802, 0xff}, /*所有的时序都手动设置，而不是自动计算*/
//	{0x4805, 0x30}, /*LPX 手动设置*/
//	{0x480a, 0x04}, /*BIT ORDER REVERSE*/
//	{0x480a, 0x05},
//	{0x4801, 0x1f},
...
{
	if (code->pad || code->index >= ARRAY_SIZE(ov5645_codes))
		return -EINVAL;

	code->code = ov5645_codes[code->index];
	return 0;
}
		       ...
		       struct v4l2_subdev_frame_size_enum *fse)
{
	...
	j = ARRAY_SIZE(ov5645_codes);
	while(--j)
		if(fse->code == ov5645_codes[j])
			break;

	for (i = 0; i < N_WIN_SIZES; i++) {
		if (index == ++num_valid) {
			fse->code = ov5645_codes[j];
