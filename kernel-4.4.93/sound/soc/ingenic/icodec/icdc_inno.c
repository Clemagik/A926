...
static void inno_dac_configure(struct snd_soc_codec *codec,
...
static void inno_adc_configure(struct snd_soc_codec *codec,
			struct snd_pcm_hw_params *params)
{
	int fmt_width = snd_pcm_format_width(params_format(params));
	unsigned int reg = 0x00;
			    ...
			    struct snd_soc_dai *dai)
{
	...
	*(volatile unsigned int *)(INNO_CODEC_BASE + 0x03*4) = reg;
	...
	if(substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		inno_dac_configure(codec, params);
	} else {
		inno_adc_configure(codec, params);
	}
...
int icodec_mute_stream(struct snd_soc_dai *dai, int mute, int stream)
{
//	struct snd_soc_codec *codec = dai->codec;
...
static struct snd_soc_dai_ops icodec_dai_ops = {
	...
	.mute_stream	= icodec_mute_stream,
