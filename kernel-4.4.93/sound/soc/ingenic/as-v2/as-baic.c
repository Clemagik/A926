				...
				struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
	...
	u32 ctrl_reg = is_out ? BAICTCTL(id) : BAICRCTL(id);
	...
	/*reset baic controller*/
	regmap_write(baic->regmap, ctrl_reg, BAICCTR_RST);
...
static int ingenic_baic_platform_probe(struct platform_device *pdev)
{
	...
	/* For I2S clk enable failed */
	*(volatile unsigned int *)(0xb0000060) |= (1<<29);
	*(volatile unsigned int *)(0xb000007c) |= (1<<29);
	*(volatile unsigned int *)(0xb0000084) |= (1<<29);
	*(volatile unsigned int *)(0xb000008c) |= (1<<29);

	ret = of_property_read_u32(pdev->dev.of_node, "ingenic,baic23-interface-vol", &interfac_vol);
	if(!ret) {
		if(interfac_vol == BAIC23_INTERFACE_VOL_1V8)
			*(volatile unsigned int *) 0xb00000E0 |= (1 << 30);
		if(interfac_vol == BAIC23_INTERFACE_VOL_3V3)
			*(volatile unsigned int *) 0xb00000E0 &= ~(1 << 30);
	} else {
		dev_info(&pdev->dev, "Baic23 interface is the default voltage\n");
	}
